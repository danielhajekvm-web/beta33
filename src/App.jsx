        ...
        <div className={cardClass}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div className="bg-indigo-50 rounded-xl p-4 shadow-sm">
              ...
              <div className="text-2xl font-bold">{filteredTransactions.reduce((s,t)=> s + (Number(t.purchasePricePln||0)*exchangeRate),0).toLocaleString("cs-CZ",{minimumFractionDigits:0,maximumFractionDigits:0})}</div>
            </div>
            <div className="bg-emerald-50 rounded-xl p-4 shadow-sm">
              <div className="text-sm text-gray-600">Prodej (CZK)</div>
              <div className="text-2xl font-bold">{filteredTransactions.reduce((s,t)=> s + Number(t.sellingPriceCzk||0),0).toLocaleString("cs-CZ",{minimumFractionDigits:0,maximumFractionDigits:0})}</div>
            </div>
            <div className="bg-yellow-50 rounded-xl p-4 shadow-sm">
              <div className="text-sm text-gray-600">Čistý zisk (CZK)</div>
              <div className="text-2xl font-bold">{filteredTransactions.reduce((s,t)=> s + Number(t.netProfitCzk||0),0).toLocaleString("cs-CZ",{minimumFractionDigits:0,maximumFractionDigits:0})}</div>
            </div>
                  ...
                  <tr key={item.id}>
                    ...
                    <td className={tdClass}>
                      <div className="flex space-x-2">
                        ...
                        <button onClick={() => handleAddToReturns(item)} className="text-amber-600 hover:text-amber-800" title="Přidat do vrácení">
  ...
  const DeliveryFilterView = ({ transactions, drivers }) => {
    ...
    const handlePrint = () => {
      const printContents = `
        <html>
          <head>
            <title>Seznam rozvozů pro ${'${selectedDriver}'} - ${'${weekNumber}'}</title>
            <style>
              body { font-family: 'Inter', sans-serif; margin: 20px; color: #333; }
              h1 { font-size: 28px; margin-bottom: 20px; color: #1e3a8a; }
              h2 { font-size: 20px; margin-bottom: 20px; color: #4b5563; }
          ...
          <body>
            <h1>Seznam rozvozů</h1>
            <h2>Řidič: ${'${selectedDriver}'} (${'${weekNumber}'})</h2>
            <table>
              <thead>
                <tr>
                  <th>Položka</th>
        ...
        <div className={containerClass}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div className="bg-indigo-50 rounded-xl p-4 shadow-sm">
              <div className="text-sm text-gray-600">Nákup (CZK)</div>
              <div classNam
  const ReturnsView = ({ returnsList, db }) => {
    const [weekOffset, setWeekOffset] = useState(0);
    const today = new Date();
    const startOfCurrentWeek = getStartOfWeek(today);
    const targetWeekStart = new Date(startOfCurrentWeek);
    ...
    const weeklyReturns = returnsList.filter(r => {
      if (!r.createdAt) return false;
      const d = r.createdAt.toDate ? r.createdAt.toDate() : new Date(r.createdAt);
      return d >= targetWeekStart && d < endOfTargetWeek;
    });

    const handleToggleReturned = async (row, value) => {
      const ref = doc(db, `/artifacts/${__app_id}/public/data/returns`, row.id);
      await updateDoc(ref, { returned: value, returnedAt: value ? serverTimestamp() : null });
    };

    const handleSaveDeposit = async (row, value) => {
      const ref = doc(db, `/artifacts/${__app_id}/public/data/returns`, row.id);
      await updateDoc(ref, { depositCzk: Number(value||0) });
    };
        ...
        <div className={cardClass}>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-2">
              <button onClick={() => setWeekOffset(prev => prev - 1)} className="px-3 py-2 rounded-xl bg-gray-100 hover:bg-gray-200">Předchozí týden</button>
              <span className="text-gray-700 font-medium">T{weekNumber}</span>
              <button onClick={() => setWeekOffset(prev => prev + 1)} className="px-3 py-2 rounded-xl bg-gray-100 hover:bg-gray-200">Další týden</button>
            </div>
          </div>
            ...
            <table className={tableClass}>
              <thead className={headerClass}>
                <tr>
                  ...
                  <th className={thClass}>Záloha (Kč)</th>
                  <th className={thClass}>Stav</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {weeklyReturns.map(row => (
                  <tr key={row.id} className={row.returned ? 'bg-green-50' : ''}>
                    <td className={tdClass}>{row.itemName || '-'}</td>
                    <td className={tdClass}>{row.note || '-'}</td>
                    <td className={tdClass}>{Number(row.sellingPriceCzk||0).toLocaleString("cs-CZ",{minimumFractionDigits:0,maximumFractionDigits:0})}</td>
                    <td className={tdClass}>{row.deliveryCity || '-'}</td>
                    <td className={tdClass}>{row.customerAddress || '-'}</td>
                    <td className={tdClass}>
                      <div>{row.customerContact || '-'}</div>
                      {row.customerPhone2 ? <div className="text-xs text-gray-600">{row.customerPhone2}</div> : null}
                    </td>
                    <td className={tdClass}>
                      <input type="number" defaultValue={row.depositCzk||0} className="w-28 p-2 border rounded-xl" onBlur={(e)=>handleSaveDeposit(row,e.target.value)} />
                    </td>
                    <td className={tdClass}>
                      <select value={row.returned ? 'vraceno':'nesplneno'} onChange={(e)=>handleToggleReturned(row, e.target.value==='vraceno')} className="p-2 border rounded-xl">
...
e="text-2xl font-bold">{filteredTransactions.reduce((s,t)=> s + (Number(t.purchasePricePln||0)*exchangeRate),0).toLocaleString("cs-CZ",{minimumFractionDigits:0,maximumFractionDigits:0})}</div>
            </div>
            <div className="bg-emerald-50 rounded-xl p-4 shadow-sm">
              <div className="text-sm text-gray-600">Prodej (CZK)</div>
              <div className="text-2xl font-bold">{filteredTransactions.reduce((s,t)=> s + Number(t.sellingPriceCzk||0),0).toLocaleString("cs-CZ",{minimumFractionDigits:0,maximumFractionDigits:0})}</div>
  ...
  return (
    <div className="bg-gray-100 min-h-screen font-sans">
        ...
        <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
          <div className="relative flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
              ...
              <button onClick={() => setCurrentView(Views.RETURNS)} className={`flex items-center px-4 py-2 rounded-xl transition-colors duration-200 ${currentView === Views.RETURNS ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-800 hover:bg-gray-200'}`}>
      ...
      <main className="py-8">
        ...
        {currentView === Views.RETURNS && <ReturnsView returnsList={returnsList} db={db} />}
